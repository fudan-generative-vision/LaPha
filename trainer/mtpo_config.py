...
class MTPOConfig(TrainingArguments):
    ...
    depth: int = field(
        default=4,
        metadata={"help": "Max depth of ReAct / tree search."},
    )
    breadth: int = field(
        default=4,
        metadata={"help": "Breadth (children per node) for plain BFS ReAct search."},
    )

    num_sim: int = field(
        default=128,
        metadata={"help": "Number of Monte-Carlo simulations per search."},
    )
    num_pos_sim: int = field(
        default=4, 
        metadata={"help": "Stop the simulation when `num_pos_sim` positive leaves are reached"}
    )
    prune_per: int = field(
        default=16,
    )
    c_puct: float = field(
        default=1.4,
        metadata={
            "help": "MCTS: exploration coefficient in the UCB formula "
                    "`Q + c_puct * prior * sqrt(ln(N_parent)/(1+N_child))`."
        },
    )
    value_head_type: str = field(
        default="qwen2", 
    )   
    ...
    distance_alpha: float = field(
        ...
        metadata={
            "help": (
                "Strength of distance shaping. Larger -> stronger decay with distance. "
                "Used in exp(-alpha * normalized_distance)."
            )
        },
    )
    passk_k: int = field(
        default=16,
        metadata={"help": "Top-K used when computing pass@K on the eval dataset."},
    )
    passk_threshold: float = field(
        default=1.0,
        metadata={
            "help": (
                "Threshold to mark a leaf as 'correct' when computing pass@K and "
                "distance-to-success. Typical 1.0 for exact correctness; "
                "use 0.6 if reward is soft in [0,1]."
            )
        },
    )
    enable_thinking: bool = field(
        default=False,
        metadata={
            "help": "Whether to use reasoning model chat template."
        },
    )
    ...
    def __post_init__(self):
        ...
        # The generation batch must contain full prompt groups (no partials), so it must be divisible by num_generations.
