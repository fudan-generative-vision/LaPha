...
MAX_EXPONENT_ABS = 10**6
MAX_BITSHIFT_ABS = 10**6
MAX_RANGE_LENGTH = 10**7
def evaluate_binop(
    binop: ast.BinOp,
    state: dict[str, Any],
    static_tools: dict[str, Callable],
    ...
    if isinstance(binop.op, ast.Add):
        ...
        if isinstance(right_val, (int, float)) and abs(right_val) > MAX_EXPONENT_ABS:
            raise InterpreterError(
                f"Exponent too large ({right_val}); refusing to compute such a big power (**)."
            )
        if isinstance(left_val, int) and isinstance(right_val, int):
            try:
                base_digits = len(str(abs(left_val)))
            except Exception:
                base_digits = 1
            if base_digits * abs(right_val) > MAX_EXPONENT_ABS:
                raise InterpreterError(
                    "Integer power would create an extremely large number; refusing to compute."
                )
        return left_val ** right_val
    elif isinstance(binop.op, ast.FloorDiv):
        return left_val // right_val
        ...
        if isinstance(right_val, int) and abs(right_val) > MAX_BITSHIFT_ABS:
            raise InterpreterError(
                f"Shift count too large ({right_val}); refusing to compute such a big bit shift (<<)."
            )
        return left_val << right_val
    elif isinstance(binop.op, ast.RShift):
        if isinstance(right_val, int) and abs(right_val) > MAX_BITSHIFT_ABS:
...
def evaluate_call(
    ...
    #     return func(*args, **kwargs)
        ...
        if func_name == "pow" and len(args) >= 2:
            exp = args[1]
            if isinstance(exp, (int, float)) and abs(exp) > MAX_EXPONENT_ABS:
                raise InterpreterError(
                    f"Exponent too large ({exp}); refusing to compute such a big power with pow()."
                )

        if func_name == "range" and len(args) >= 1:
            est_len = None
            try:
                if len(args) == 1:
                    est_len = int(args[0])
                    ...
                    est_len = int(stop) - int(start)
                else:
                    start, stop, step = args[0], args[1], args[2]
                    step = int(step)
                    if step == 0:
                        est_len = MAX_RANGE_LENGTH + 1
                    else:
                        est_len = int((int(stop) - int(start)) / step)
                if est_len is not None and est_len > MAX_RANGE_LENGTH:
                    raise InterpreterError(
                        f"range() would produce too many elements (~{est_len}); refusing to execute."
                    )
            except Exception:
                pass
